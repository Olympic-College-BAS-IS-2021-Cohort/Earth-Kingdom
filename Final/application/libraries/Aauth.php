...
class Aauth {
	 ...
	 */
	public function create_user($email, $pass, $username) {
		...
		if ( $this->aauth_db->insert($this->config_vars['users'], $data )){
			...
			if($this->config_vars['verification']){
				...
				$this->send_verification($user_id, $username);
			}
	 ...
	 */
	public function send_verification($user_id, $username = ''){
		...
		if ($query->num_rows() > 0){
			...
			if(!empty($username)) {
				$optionalUsername = 'Your username is: '. $username;
				} else {
					$optionalUsername = '';
				}
			...
			$this->CI->email->message(
				$this->CI->lang->line('aauth_email_verification_code') . $ver_code . "\r\n" . $optionalUsername .
				$this->CI->lang->line('aauth_email_verification_text') . $this->config_vars['verification_link']);
			$this->CI->email->send();
		}
	}
